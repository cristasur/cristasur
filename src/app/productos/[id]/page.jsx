...
export default async function ProductDetail({ params }) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-4 py-4 md:py-10">
      ...
      {/* También disponible en — misma familia (por etiqueta) */}
      {sameFamily?.length > 0 && (
          ...
          <div className="flex flex-wrap gap-3">
            {sameFamily.map((p) => (
              <a
                ...
                className="flex items-center gap-2.5 px-3 py-2 rounded-xl border border-slate-200 bg-white hover:border-brand-400 hover:shadow-sm transition-all group"
              >
                {p.image ? (
                  <img
                    src={p.image}
                ...
                <div>
                  <div className="text-sm font-semibold text-slate-800 group-hover:text-brand-700 line-clamp-1 max-w-[160px]">
                    {p.name}
                  </div>
